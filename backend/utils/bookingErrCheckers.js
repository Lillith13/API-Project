...
async function bookingConflicts(req, _res, next) {
  ...
  if(booking) {
    if(startYear === startDate.getFullYear() || endYear === endDate.getFullYear()) {
      ...
      if((startDate.getMonth() >= startMonth && startDate.getDate() >= startDay) && (startDate.getMonth() <= endMonth && startDate.getDate() <= endDay)) {
        err.errors.startDate = "Start date conflicts with an existing booking";
        errTriggered = true;
      }
      // if desired endDate falls within an already booked timeFrame
      if((endDate.getMonth() >= startMonth && endDate.getDate() >= startDay) && (endDate.getMonth() <= endMonth && endDate.getDate() <= endDay)) {
  ...
  if (errTriggered) {
    err.message = "Sorry, this spot is already booked for the specified dates, please check the spot's listed bookings and choose a timeframe outside of what has already been booked.";
    err.status = 403;
    next(err);
  }
  next();
